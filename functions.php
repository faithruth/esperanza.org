...
if ( ! function_exists( 'esperanza_setup' ) ) :
	...
	function esperanza_setup() {
		 ...
		 */
		load_theme_textdomain( 'esperanza', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'esperanza' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'esperanza_custom_background_args',
				array(
 ...
 */
function esperanza_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'esperanza_content_width', 640 );
}
add_action( 'after_setup_theme', 'esperanza_content_width', 0 );
...
function esperanza_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'esperanza' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'esperanza' ),
